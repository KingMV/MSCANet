...
def random_crop(img, den, dst_size):
    ...
    # ts_hd, ts_wd = den.shape
    # ts_hd = ts_hd * cfg_data.LABEL_FACTOR
    # ts_wd = ts_wd * cfg_data.LABEL_FACTOR
    # print(ts_hd, '\t', ts_wd)
...
def SHHA_collate(batch):
    ...
    if isinstance(imgs[0], torch.Tensor) and isinstance(dens[0], torch.Tensor):
        ...
        min_ht = (min_ht // cfg_data.LABEL_FACTOR) * cfg_data.LABEL_FACTOR // 2
        min_wd = (min_wd // cfg_data.LABEL_FACTOR) * cfg_data.LABEL_FACTOR // 2
...
def loading_data():
    ...
    img_transform = standard_transforms.Compose([
        # own_transforms.IMGScale(factor),
        standard_transforms.RandomRotation(),
    ...
    train_set = SHHA(os.path.join(cfg_data.DATA_PATH, 'ShanghaiTech', 'part_A', 'train_data'), 'train',
    ...
    val_set = SHHA(os.path.join(cfg_data.DATA_PATH, 'ShanghaiTech', 'part_A', 'test_data'), 'test', main_transform=None,
                   img_transform=img_transform, gt_transform=gt_transform)
