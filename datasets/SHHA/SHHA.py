...
class SHHA(data.Dataset):
    def __init__(self, data_path, mode, main_transform=None, img_transform=None, gt_transform=None):
        self.img_path = data_path + '/train_images'
        self.gt_path = data_path + '/adaptive_0.0001'
    ...
    def read_image_and_gt(self, fname):
        ...
        if img.mode == 'L':
            img = img.convert('RGB')
        w, h = img.size
        img = img.resize((w // 8 * 8, h // 8 * 8), Image.BICUBIC)
        ...
        den = cv2.resize(den, (den.shape[1] // 8, den.shape[0] // 8), interpolation=cv2.INTER_LINEAR) * 64
